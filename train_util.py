...
from flowrl_sampler import FlowRLSampler
...
class BaseDataset(torch.utils.data.Dataset):
	def __init__(
		...
		self.flowrl: Optional[object] = None

		# caching
		self.caching_mode = None  # None, 'latents', 'text'

	def attach_flowrl(self, sampler) -> None:
		"""Attach a FlowRL sampler that can plan per-epoch batch order."""
		self.flowrl = sampler
	...
	def shuffle_buckets(self):
		...
		if getattr(self, "flowrl", None) is not None:
			# Expect a method that returns a list[BucketBatchIndex] for this epoch.
			# Provide rich context so the policy can balance tags, styles, etc.
			self.buckets_indices = self.flowrl.build_epoch_indices(
				buckets_indices=self.buckets_indices,        # current all-batches list
				bucket_manager=self.bucket_manager,          # buckets & image keys
				image_data=self.image_data,                  # captions, repeats, flags
				image_to_subset=self.image_to_subset,        # per-subset options
				tag_frequency=self.tag_frequency,            # per-dir tag stats
				epoch=self.current_epoch,                    # for curriculum-like logic
				step_count=self._length if hasattr(self, "_length") else None,
			)
		else:
			# default behavior
...
class DatasetGroup(torch.utils.data.ConcatDataset):
	...
	def attach_flowrl(self, sampler):
		for d in self.datasets:
			if hasattr(d, "attach_flowrl"):
				d.attach_flowrl(sampler)
