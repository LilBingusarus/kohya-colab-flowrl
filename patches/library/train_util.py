...
try:
	from .flowrl_sampler import FlowRLSampler  # optional
except Exception:
	FlowRLSampler = None
...
class BaseDataset(torch.utils.data.Dataset):
	def __init__(
		...
		self.flowrl = None  # optional FlowRL epoch planner

	def attach_flowrl(self, sampler):
		self.flowrl = sampler
	...
	def shuffle_buckets(self):
		...
		# Lazy attach from env to avoid touching higher-level scripts/notebooks
		if self.flowrl is None and os.getenv("FLOWRL_ENABLE", "0") == "1" and FlowRLSampler is not None:
			try:
				self.attach_flowrl(FlowRLSampler.from_env())
				print(f"[FlowRL] enabled: T={self.flowrl.T} C={self.flowrl.C} W={self.flowrl.W}")
			except Exception as e:
				print(f"[FlowRL] init failed: {e}")

		# If attached, ask FlowRL to plan this epoch; fall back on any error
		if self.flowrl is not None:
			try:
				planned = self.flowrl.build_epoch_indices(
					buckets_indices=self.buckets_indices,
					bucket_manager=self.bucket_manager,
					image_data=getattr(self, "image_data", None),
					epoch=getattr(self, "current_epoch", 0),
				)
				if planned and len(planned) == len(self.buckets_indices):
					assert set(planned) == set(self.buckets_indices), "[FlowRL] planned order changed elements"
					self.buckets_indices = planned
					return
			except Exception as e:
				print(f"[FlowRL] planning failed: {e} — fallback shuffle")
	...
	def __getitem__(self, index):
		...
		for image_key in bucket[image_index : image_index + bucket_batch_size]:
			...
			if tokenization_required:
				...
				if ts is None:
				    raise RuntimeError("TokenizeStrategy not set yet. Call set_current_strategies() before indexing the dataset.")
				input_ids = [ids[0] for ids in ts.tokenize(caption)]
